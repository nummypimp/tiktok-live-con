...
import RealSlotGame from "./RealSlotGame";
...
export default function PlayerLobbyAndSlotMachine2() {
  ...
  useEffect(() => {
    const statusListener = (data) => {
      ...
      if (!data.accepting && players.length > 0) setGameReady(true);
    ...
    {shuffle(players).map((p, i) => (
      ...
      { /*!acceptingPlayers && gameReady && pool.length > 0 && ( */
       pool.length > 0 && (
        <div>
          <hr />
          <RealSlotGame players={shuffle(players)} />
        </div>
      )}
    </div>
  );
